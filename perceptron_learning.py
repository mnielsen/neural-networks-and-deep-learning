...
class Perceptron():
    ...
    ``learn`` a bias and set of weights that compute that function, using
    the perceptron learning algorithm. """

    def __init__(self, num_inputs=2):
        """ Initialize the perceptron with the bias and all weights set
        to 0.0. ``num_inputs`` is the number of input bits to the
        perceptron."""
        self.num_inputs = num_inputs
        self.bias = 0.0
        self.weights = numpy.zeros((num_inputs,1))
        # self.inputs is a convenience attribute.  It's a list containing
        # all possible binary inputs to the perceptron.  E.g., for three
        # inputs it is: [[0, 0, 0], [0, 0, 1], ...]
        self.inputs = [[int(y)
    ...
    def output(self, x):
        ...
        ``x``."""
        return 1 if sum(wt*y for (wt, y) in zip(self.weights, x)) + \
               self.bias > 0 else 0
    ...
    def learn(self, f, eta=0.1):
        ...
        should be a small positive number.  Does not terminate when the
        function cannot be computed using a perceptron."""        
        # initialize the bias and weights with random values
        self.bias = random.gauss(0.0, 1.0)
        self.weights = [random.gauss(0.0, 1.0)
                        for x in xrange(self.num_inputs)]
        number_of_errors = -1
        while number_of_errors != 0:         
            ...
            for x in self.inputs:
                ...
                if error:
                    ...
                    self.weights = [self.weights[j]+eta*error*x[j]
