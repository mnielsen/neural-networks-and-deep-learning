...
class Network():
    """
    The Network class models a feedforward neural nets.

    It's easy to make a neural net!

    >>> # A Neural Net with 200 inputs,
    >>> # 100 hidden units, 200 outputs
    >>> Network([200, 100, 200])
    Network([200, 100, 200])

    Training a net is easy, too!
    Let's train a net to act as an XOR gate.
    ...
    >>>
    >>> net = Network([2,3,1])
    >>> # Now we apply Stochastic Gradient Descent
    >>> net.SGD(train_data, 1000, 2, 0.4, 0)
    ...
    Though, for best performance, you should
    use numpy arrays instead:

    >>> import numpy as np
    >>> x = np.asarray([0,0]).reshape([-1, 1])
    >>> x
    array([[0],
           [0]])
    >>> net(x)
    array([[0.0052980347655956711]])

    You know what else is easy?
    Saving and loading neural nets!

    (I know, you're starting to get it,
     everything is easy in this library!)
    ...
    The library provides an number of methods
    to facilitate easy pretraining.

    The key analogy for these is thinking of
    the net as a list of layers.

    You can train small nets, splice part of
    them out, and combine nets with layers of
    the same size.

    >>> auto = Network([20, 10, 20])
    >>> len(auto)
    3
    >>> auto[:2]
    Network([20, 10])
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        also be supplied.
        """
        if type(training_data[0][0]) == list or type(training_data[0][0]) == tuple:
            training_data = training_data[:]
            for x in training_data:
                x[0] = np.asarray(x[0]).reshape(len(x[0]), 1)
                x[1] = np.asarray(x[1]).reshape(len(x[1]), 1)
            print training_data
    ...
    def __repr__(self):
        """Pretty representations of Network instances."""
        return "Network(%s)" % self.sizes

    def __call__(self, a):
        """Feedforward operations on a neural net are effectively evaluation..."""
        if len(a) != self.sizes[0]:
            raise Exception("Neural net input must be the same size as input layer")
        if type(a) == list or type(a) == tuple:
            a = np.asarray(a).reshape(len(a), 1)
            x = list(self.feedforward(a).reshape(-1))
            return x
        else:
            return self.feedforward(a)

    def __add__(self, a):
        if self.sizes[-1] != a.sizes[0]:
            raise Exception("Concatonating neural nets A and B requires\
                             output layer of A be the same as input layer of B")
        result = Network(self.sizes + a.sizes[1:])
        result.biases = self.biases + a.biases
        result.weights = self.weights + a.weights
        return result

    def __getslice__(self, i, j):
        result = Network(self.sizes[i:j])
        result.biases = self.biases[i:j-1]
        result.weights = self.weights[i:j-1]
        return result
    ...
    def save(self, fname):
        net_data = (self.sizes, self.biases, self.weights)
        with open(fname, "wb") as f:
            cPickle.dump(net_data, f)

    def load(self, fname):
        with open(fname, "rb") as f:
            net_data = cPickle.load(f)
        self.sizes, self.biases, self.weights = net_data
